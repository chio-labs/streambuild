...
export function layoutGrouped(
	...
	for (const [key, members] of membersByGroup) {
		const inner: DagreGraph = new dagre.graphlib.Graph();
		inner.setGraph({
			rankdir: 'LR',
		...
		const memberIds: Set<string> = new Set(members.map((node) => node.id));
		...
		for (const node of members) {
			const pos: { x: number; y: number } = inner.node(node.id);
			const { width, height } = sizeOf(node, opts);
	...
	const outer: DagreGraph = new dagre.graphlib.Graph();
	outer.setGraph({
		rankdir: 'LR',
	...
	for (const [key, size] of groupSize) {
		const pos: { x: number; y: number } = outer.node(key);
		boxes.push({
			id: groupNodeId(key),
			groupKey: key,
