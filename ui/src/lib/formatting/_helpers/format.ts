...
function formatDuration(seconds: number): string {
	...
	const hours: number = Math.round((seconds % 86400) / 3600);
 ...
 */
function parseUtc(instant: string): Date {
	const isoShaped: string = instant.includes('T') ? instant : instant.replace(' ', 'T');
	const zoned: string = /(?:[Zz]|[+-]\d\d:?\d\d)$/.test(isoShaped)
		? isoShaped
		: `${isoShaped}Z`;
...
function formatAgo(instant: string | null, reference: string): string {
	if (!instant) return '—';
	const seconds: number = secondsBetween(instant, reference);
...
function formatEngineFamily(engine: string | null): string {
	if (engine === null) return 'TABLE';
	const argumentStart: number = engine.indexOf('(');
