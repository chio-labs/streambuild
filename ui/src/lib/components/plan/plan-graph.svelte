<script lang="ts">
	...
	import { buildLogicalGraph, modelByName } from '$lib/domain/derive';
	import type { Graph, Plan, Project } from '$lib/domain/types';
	...
	const graph = $derived<Graph>({
		...
		edges: full.edges.filter((edge) => {
			const source = full.nodes.find((node) => node.id === edge.source);
			const target = full.nodes.find((node) => node.id === edge.target);
			if (!source || !target) return false;
			const keep = (name: string): boolean => inScope.has(name) || heldBack.has(name);
	...
	$effect(() => {
		...
		const timer = setTimeout(() => fitView?.(FIT), 80);
