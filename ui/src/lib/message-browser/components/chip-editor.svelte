	...
	{#if open}
		<div class="bg-background absolute left-0 top-[26px] z-20 w-[300px] rounded-[4px] border border-border p-2.5 shadow-lg">
			...
			{:else}
				...
				{#if field === 'json'}
					<input bind:value={path} placeholder="json path, e.g. data.source" class="bg-[var(--sb-inset)] mb-1.5 w-full rounded-[4px] border border-border px-2 py-1 font-mono text-[11px] outline-none focus:border-[var(--primary)]" />
				{/if}
				{#if needsValue}
