<script lang="ts">
	...
	import type { MessagePredicate, PredicateField } from './types';
	...
	/** JSON paths are typed dotted, e.g. `data.placer` or `items.0.id`. */
...
<div class="relative">
	<button
		class="text-muted-foreground hover:text-foreground flex items-center gap-1 rounded-[4px] border border-dashed border-border px-2 py-[3px] font-mono text-[10.5px]"
		onclick={() => (open = !open)}><PlusIcon size={11} /> filter</button
	>
	{#if open}
		<div
			class="bg-background absolute left-0 top-[26px] z-20 w-[300px] rounded-[4px] border border-border p-2.5 shadow-lg"
		>
			<div class="flex flex-wrap gap-1 pb-2">
				{#each Object.keys(OPS_BY_FIELD) as candidate (candidate)}
					<button
						class="rounded-[4px] border px-2 py-[2px] font-mono text-[10.5px] {field === candidate
							? 'border-[var(--primary)] text-foreground'
							: 'text-muted-foreground border-border hover:text-foreground'}"
						onclick={() => pickField(candidate as PredicateField)}>{candidate}</button
					>
				{/each}
			</div>
			...
			{#if field === 'partition'}
				<div class="flex max-h-[120px] flex-wrap gap-1 overflow-auto pb-2">
					{#each knownPartitions as partition (partition)}
						<button
							class="rounded-[4px] border px-1.5 py-[1px] font-mono text-[10.5px] {selectedPartitions.includes(
								partition
							)
								? 'border-[var(--primary)] text-foreground'
								: 'text-muted-foreground border-border'}"
							onclick={() => togglePartition(partition)}>{partition}</button
						>
					{/each}
				</div>
			{:else}
				<div class="flex gap-1 pb-2">
					{#each ops as candidate (candidate)}
						<button
							class="rounded-[4px] border px-1.5 py-[1px] font-mono text-[10px] {op === candidate
								? 'border-[var(--primary)] text-foreground'
								: 'text-muted-foreground border-border hover:text-foreground'}"
							onclick={() => (op = candidate)}
							title={OP_HINT[candidate]}>{OP_HINT[candidate]}</button
						>
					{/each}
				</div>
				{#if field === 'json'}
					<input
						bind:value={path}
						placeholder="json path, e.g. data.placer"
						class="bg-[var(--sb-inset)] mb-1.5 w-full rounded-[4px] border border-border px-2 py-1 font-mono text-[11px] outline-none focus:border-[var(--primary)]"
					/>
				{/if}
				{#if needsValue}
					<input
			...
			<div class="flex justify-end gap-1.5">
				<button
					class="text-muted-foreground hover:text-foreground rounded-[4px] border border-border px-2 py-[3px] font-mono text-[10.5px]"
					onclick={() => (open = false)}>cancel</button
				>
				<button
					class="rounded-[4px] border border-[var(--primary)] px-2 py-[3px] font-mono text-[10.5px] disabled:opacity-40"
					disabled={!ready}
