...
from streambuild.cli.readiness.main._format_bool import format_bool
from streambuild.cli.readiness.main._format_count import format_count
from streambuild.cli.readiness.main._format_percentage import format_percentage
from streambuild.cli.readiness.main._format_range import format_range
...
def render_deployment_audit_json(*, result: DeploymentAuditResult, project_dir: Path | None) -> str:
...
def render_deployment_audit_text(
    *, result: DeploymentAuditResult, database: str, project_dir: Path | None
) -> str:
...
def _render_audit_header(*, result: DeploymentAuditResult, database: str) -> list[str]:
    lines: list[str] = [
...
def _render_next_steps(result: DeploymentAuditResult) -> list[str]:
    lines: list[str] = ["", cli_style().section("Next")]
    if result.assessment == AuditAssessment.READY:
        lines.append(f"- stb deployment promote {result.deployment_id}")
        return lines
