"""Render standard-mode plans as operator text and machine-readable JSON."""
...
def render_standard_plan_json(*, plan: StandardPlan, adapter_name: str) -> str:
    """Render one standard plan as deterministic JSON."""
...
def _render_header(*, plan: StandardPlan, adapter_name: str) -> tuple[str, ...]:
    return (
        cli_style().title("Standard Plan"),
...
def _render_execution_scope(*, plan: StandardPlan) -> tuple[str, ...]:
    return (
...
def _render_prerequisites(*, plan: StandardPlan) -> tuple[str, ...]:
    return (
...
def _render_replay_roots(*, plan: StandardPlan) -> tuple[str, ...]:
    lines: list[str] = []
...
def _render_replay_root(*, root: StandardReplayRoot) -> tuple[str, ...]:
...
def _render_destructive_actions(*, plan: StandardPlan) -> tuple[str, ...]:
    return (
...
def _render_warnings(*, plan: StandardPlan) -> tuple[str, ...]:
    return (
...
def _prerequisite_payload(prerequisite: StandardPrerequisite) -> dict[str, object]:
    return {
...
def _entry_payload(entry: StandardPlanEntry) -> dict[str, object]:
    return {
...
def _replay_root_payload(root: StandardReplayRoot) -> dict[str, object]:
    return {
...
def _operation_payload(operation: StandardRelationOperation) -> dict[str, object]:
    return {
