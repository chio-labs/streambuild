...
class SqlTestChainAssembler:
    ...
    def assembled_position(self, logical_name: str) -> int:
        ...
        cte_name: str = self._assembled_cte_by_name[logical_name]
        return next(
            index for index, (name, _query) in enumerate(self._assembled_ctes) if name == cte_name
        )
    ...
    def resolve(self, *, logical_name: str) -> str:
        ...
        return self._resolve(logical_name=logical_name, stack=frozenset())
    ...
    def _resolve(self, *, logical_name: str, stack: frozenset[str]) -> str:
        ...
        assembled_name: str | None = self._assembled_cte_by_name.get(logical_name)
        if assembled_name is not None:
        ...
        entry: CompiledModel | None = self._registry.get(logical_name)
        if entry is None:
            raise SqlTestAssemblyError(self._unresolved_message(logical_name))
        return self._assemble(logical_name=logical_name, entry=entry, stack=stack)

    def _assemble(self, *, logical_name: str, entry: CompiledModel, stack: frozenset[str]) -> str:
        resolver: dict[str, str] = {}
        parsed_ref: ParsedRef
        for parsed_ref in entry.parsed_refs:
            resolver[parsed_ref.name] = self._resolve(
                logical_name=parsed_ref.name, stack=stack | {logical_name}
            )
