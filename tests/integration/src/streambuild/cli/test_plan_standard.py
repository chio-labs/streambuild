...
from tests.integration.src.streambuild.cli._test_types import CliStandardPlanIntegrationTestCase
from tests.integration.src.streambuild.cli.helpers import (
    ...
    plan_scope_names,
    run_standard_plan,
    settle_standard_scope_warehouse,
)
...
@pytest.mark.parametrize(
    "test_case",
    [
        CliStandardPlanIntegrationTestCase(
            ...
            expected_settled_ownership=("standard",),
        )
    ],
    ids=lambda case: case.description,
)
def test_given_settled_standard_warehouse_when_planning_twice_then_closure_never_shrinks(
    test_case: CliStandardPlanIntegrationTestCase,
    tmp_path: Path,
    ...
    try:
        ...
        initial_exit_code: int = run_standard_plan(
        ...
        first_exit_code: int = run_standard_plan(
            project_root=tmp_path, database=clickhouse_database, connection=connection
        )
        first_output: str = capsys.readouterr().out
        second_exit_code: int = run_standard_plan(
            project_root=tmp_path, database=clickhouse_database, connection=connection
        )
